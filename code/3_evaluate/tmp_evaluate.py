...
def get_s(size_x, lx):
    ...
    #    if (size_x - lx) % sx == 0:
    #        return sx
    # where the heck is 16 coming from..
    return lx - 16
...
def clark_get_diagnostic_data(defect_dir_list, verbose=False):
    data_list = []
...
def plot_diagnostics(data_list, label_list, save_dir, diag="loss", log=True, invert=False, N=1, save=False, prefix=''):
    ...
    plt.tight_layout()
    if save:
        fig.savefig(save_dir + "{}loss.png".format(prefix), dpi=500)
    plt.show()
