...
def make_data(input_dir, save_dir, data_dirs, label_list, l_shape, stride, ftype, parsed_dir_name="parsed", prefix="train",\
        AUG=True, tol=1e-5, ones_pcent=0, one_save=True, fsize=1000):

    (lx, ly) = l_shape

    parsed_dir = save_dir + parsed_dir_name + "/"
        ...
        if AUG:
            ...
            for aug_dir in os.listdir(augments):
                ...
                if not one_save and len(data) >= fsize:
                    ...
                    # So this is the part that I need to change if I am to control the number of files that are generated.
