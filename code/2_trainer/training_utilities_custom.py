...
def construct_model(N, k_fac, nb_classes, sess_dir, model_fn, model_weights_fn, dropout=0.1):
    print("Input Image shape is (N, N, 1) where N: ")
    print(N)
    input_img = Input(shape=(N,N,1))

    print("Constructing ResUNet with model_resunet")
...
def calc_accuracy(model, x_test, y_true, N, nb_classes, plots):
    ...
    for evals_img, label_img in zip(y_evals, y_trues):
        ...
        print("Getting the center lists for labels & evals")
        ...
        '''
        print("length of match_list, add this to TP: ")
        ...
        '''
        print("length of evals_list, add this to FP: ")
        print(len(match_list))
        '''
        ...
        '''
        print("length of label list, add this to FN: ")
        print(len(label_list))
        '''
    
        FN += len(label_list)

    # Where is the 304 coming from? Probably has something to do with the total number of images
...
def train_step(checkpoint_filepath, model, train_stem, test_stem, model_weights_fn, plots, callbacks_list, epochs=1, batch_size=32):
    '''
    ...
    def get_xy(batch_lbl):
        ...
        # Change this part to randomly select test_batch!!!!!
...
def train(filepath, callbacks_list, step, data_dir, N, nb_classes, model, diagnostics_fn, model_weights_fn, num_steps=-1, plots=False):
    '''
    trains continuously until force stopped
    '''
    train_list = [f for f in os.listdir(data_dir) if "train" in f]
   
    # Change this part to randomly select from multiple test sets!!!!!!
    # Below 2 lines are the original code.
    
    test_f     = data_dir + "test_00000.p"
    test_stem  = read_data_set(test_f, N, nb_classes)
    
    
    # Define directories (test & train folders separated)
    # test_dir = data_dir + test_folder
    # train_dir = data_dir + train_folder
    # test_stem = grab_data(data_dir, test_f, N, nb_classes)
    ...
    while steps_left > 0 or num_steps==-1:
        ...
        # train
        history, TP, FP, FN, TN, recall, precision, F1, bal_acc = train_step(filepath, model, train_stem,
                test_stem, model_weights_fn, plots, callbacks_list)
