...
Inputs:
    - model
    - x_test = train_stem = grab_data(data_dir, train_f, N, nb_classes)
    - y_true = test_stem = read_data_set(test_f, N, nb_classes)
    - N = image dimension (ex. 256)
    - nb_ classes: # of labels to learn (if 1 label, then nb=2)
    - plots: True or False
'''
def calc_accuracy(model, x_test, y_true, N, nb_classes, plots):
    
    # The model's predictions of where the defects are for the x_test training set
    predictions = np.reshape(np.array(model.predict_on_batch(x_test)), [-1, N, N, nb_classes])
    # Reformat predictions using np.argmax
    y_evals = np.argmax(predictions, axis=3)
    
    # The labels, or real locations of the defects (from y_true, reformat using np.argmax)
    y_trues = np.argmax(np.reshape(np.array(y_true), [-1, N, N, nb_classes]), axis=3)


    # Calculate r (What is r????)
    ...
    # For each model_image and label_image, update TP, FP, FN
    ...
    for evals_img, label_img in zip(y_evals, y_trues):
        ...
        # Detect difference
        # Compare the model's answers to the real labels, 
        # match_list = list of all defects that were matched by model within specified boundaries
        # label_list = 
        # evals_list = 
        ...
        FP += len(evals_list)
        FN += len(label_list)
    # Looks like 304 is the number of files in zip(y_evals)...    
...
def train_step(model, train_stem, test_stem, model_weights_fn, plots, epochs=1, batch_size=32):
    ...
    history = model.fit(x_train, y_train, batch_size=batch_size, epochs=epochs,\
            validation_data=(x_test, y_test), verbose=2, callbacks=[model_checkpoint_callback])
    '''
...
def train(step, data_dir, N, nb_classes, model, diagnostics_fn, model_weights_fn, num_steps=-1, plots=False):
    ...
    while steps_left > 0 or num_steps==-1:
        ...
        # Is there a way to include custom metrics in the history object?
